...
import {MenusService} from "../charisma/service/menu.service";
import {Menu} from "../charisma/api/menu";
...
export class AppMenuComponent implements OnInit {
    ...
    constructor(private userService: UserService,
        private userAuthService: UserAuthService,private menusService:MenusService
    ) {}

    ngOnInit() {
        //TODO calisma burada oalcak menu
         this.menusService.getAllMenuList().subscribe(response => {
             this.model = response.map(item => ({
                 label: item.label,
                 icon: item.icon,
                 permission: item.permission,
                 routerLink:this.parseRouterLink(item.routerLink),
                 items: (item.menuItems || []).map(subItem => ({
                     label: subItem.label,
                     icon: subItem.icon,
                     routerLink: this.parseRouterLink(subItem.routerLink),
        ...
        //     {
        //         label: 'Pizza',
        //         icon: 'assets/layout/images/flatiron/pizza.svg',
        //         permission: 'Admin',separator:true
        //
        // // items: [
        //         //     {
        //         //         label: 'Dashboards',
        ...
        //     //         },
        //     //         {
        //     //             label: 'SanaChoiceLanding',
        //     //             icon: 'pi pi-fw pi-globe',
        //     //             routerLink: ['/sanachoicelanding']
        //     //         },
        //     //         {
        //     //             label: 'Auth',
    ...
    parseRouterLink(link: any): string[] | null {
        if (!link) return null; // Null ise değişiklik yapma
        return typeof link === 'string' ? [link.replace(/^'|'$/g, '')] : link;
    }

    crateMeneuContent()
    {
        if(this.userAuthService.isAdmin()){
        this.permissionMenu = this.model.filter(object => {
    ...
    onMenuClick(item: any): void {
        ...
        console.log('RouterLink:', item.routerLink);
    }
 }
